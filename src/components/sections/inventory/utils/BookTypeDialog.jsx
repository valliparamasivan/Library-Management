...
import { BookTypeFormSchema } from "@/helpers/ValidationHelpers";
...
import { useBookTypeCreate } from "@/store/hooks/InventoryHooks";

const BookTypeDialog = ({ isOpen, onOpenChange, id, bookTypeData }) => {
  const router = useRouter();
  const isEditMode = !!id;
  const { control, handleSubmit, reset, setError, formState: { isSubmitting } } = useForm({
    resolver: zodResolver(BookTypeFormSchema),
    mode: "onSubmit",
    defaultValues: {
      bookType: "",
    },
  ...
  useEffect(() => {
    if (isOpen && bookTypeData) {
      reset({
        bookType: bookTypeData.type || bookTypeData.bookType || "",
      });
    } else if (isOpen && !id) {
      reset({
        bookType: "",
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      const response = await bookTypeCreate(payload);
      ...
      <DialogContent hideClose className="w-[calc(100%-2rem)] sm:max-w-[400px] rounded-lg p-4 sm:p-6 border-0 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-2 sm:mb-2">
          <DialogTitle className="text-base sm:text-lg font-semibold text-gray-900">
            {isEditMode ? "Edit Book Type" : "Add Book Type"}
          </DialogTitle>
          <button
