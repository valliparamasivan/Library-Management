...
const CustomerCatalogView = ({ book, reviews }) => {
  ...
  const handleToggleFavorite = async () => {
    const bookId = bookData?.bookId || params?.id;
    if (!bookId) return;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="lg:col-span-1">
            <div className="rounded-lg p-3 sm:p-4 space-y-2">
              ...
              >
                <Heart 
                  className={`w-4 h-4 sm:w-5 sm:h-5 ${isFavorite ? "text-[#D2483B]" : "text-gray-400"}`}
            ...
            <div className="rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  <span className="text-xs sm:text-sm text-gray-700">
                    {bookData?.totalRatings > 0 ? bookData.totalRatings.toFixed(1) : "0"} ({bookData?.totalRatings || 0} {(bookData?.totalRatings || 0) === 1 ? 'review' : 'reviews'})
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 py-2 sm:py-3 border-t border-b border-gray-200">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-base font-semibold text-gray-900">{bookData?.yearPublished?.toString() || "-"}</p>
                </div>
                <div>
              ...
              <div className="flex justify-start">
                <div className="bg-[#00A8841A] rounded-lg px-3 sm:px-6 lg:px-8 py-1.5 border border-[#00A88433] text-center w-full sm:w-auto">
                  <p className="text-xs sm:text-sm font-semibold text-[#00A884] pb-0.5">
                    {bookData?.availableCopies || 0} of {bookData?.quantity || 0} Available
                  </p>
                  <p className="text-xs text-[#00A884]">
                    {bookData?.available ? "Ready to borrow" : "All copies borrowed"}
                  </p>
                </div>
              </div>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="border-b border-gray-200 bg-transparent p-0 h-auto overflow-x-auto">
                  ...
                  >
                    Reviews ({bookData?.totalRatings || 0})
                  </TabsTrigger>
                </TabsList>
      ...
      <ReserveBook
        ...
        book={{
          id: params?.id || bookData?.bookId?.toString() || "1",
