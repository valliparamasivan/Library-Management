...
import CatalogFilter from "./utils/catalogFilter";
import { useAddFavorite } from "@/store/customerHooks/CatalogHooks";
import { useErrorHandler } from "@/components/custom-hooks/useErrorHandler";

const CatalogSection = ({ booksList, languages, bookCategories }) => {
  ...
  const handleToggleFavorite = async (bookId) => {
    ...
    try {
      ...
      router.refresh();
      showErrorToast(errorMessage || "Failed to add book to favorites");
    }
  };
                  ...
                  >
                    <Heart
                      className={`w-4 h-4 sm:w-5 sm:h-5 ${
                        favorites.has(book.id) ? "text-[#D2483B]" : "text-gray-400"
                      }`}
