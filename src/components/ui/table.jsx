...
function TableHeader({ className, ...props }) {
  return <thead data-slot="table-header" className={cn("[&_tr]:border-b bg-[#F6F6F6]", className)} {...props} />;
}
...
function TableCell({ className, ...props }) {
  return (
    <td
      ...
      className={cn("h-[49px] px-2 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]", className)}
      {...props}
    />
  );
}
