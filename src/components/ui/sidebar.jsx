  ...
  return (
    ...
    >
      <ImageWidget 
                 src={One} 
                 alt="Assets" 
                 className="w-6 h-6 sm:w-6 sm:h-6 transition-transform duration-500 ease-in-out rotate-180"
               />
...
function SidebarHeader({ className, ...props }) {
  return <div data-slot="sidebar-header" data-sidebar="header" className={cn("flex flex-col gap-2 p-2 h-14 shrink-0 border-b", className)} {...props} />;
}
...
function SidebarMenuItem({ className, ...props }) {
  return <li data-slot="sidebar-menu-item" data-sidebar="menu-item" className={cn("group/menu-item relative my-1", className)} {...props} />;
}

const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-sm py-3 px-4 text-left text-sm outline-hidden ring-sidebar-ring transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-[#00796B] data-[active=true]:font-medium data-[active=true]:text-white data-[active=true]:[&>svg]:!text-white data-[active=true]:[&>span]:!text-white data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-1.5! [&>span:last-child]:truncate [&>span:last-child]:transition-opacity [&>span:last-child]:duration-700 [&>span:last-child]:ease-[cubic-bezier(0.4,0,0.2,1)] [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
function SidebarMenuSub({ className, children, ...props }) {
  return (
    <ul
      data-slot="sidebar-menu-sub"
      data-sidebar="menu-sub"
      className={cn(
        "border-sidebar-border mt-2 mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-4 py-0.5 relative",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
...
function SidebarMenuSubItem({ className, children, isActive, ...props }) {
  return (
    <li data-slot="sidebar-menu-sub-item" data-sidebar="menu-sub-item" className={cn("group/menu-sub-item relative", className)} {...props}>
      {isActive && <div className="absolute -left-[21px] top-1/2 -translate-y-1/2 w-[9px] h-[9px] bg-black rounded-full z-10"></div>}
      {children}
    </li>
  );
}
